...
const getAuthState = () => getState();
const getAuthToken = () => getAuthState().token;
const getUser = () => getAuthState().user;
...
function updateAvatarUI() {
    ...
    // ✅ Priorizar: nickname → full_name → email (não "Estudante" como fallback)
...
// =========================================================
// 🌐 CONFIGURAÇÃO DE URL (detectada automaticamente)
// =========================================================
...
export class SettingsManager {
    ...
    attachEventListeners() {
        ...
        syncBtn?.addEventListener('click', async (e) => {
            ...
            try {
                ...
                if (ok) {
                    this.fillFields();
                    this.showToastSuccess('toasts.syncedTitle', 'toasts.syncedMessage');
                } else {
    ...
    async fetchLatestUserData() {
        try {
            console.log("[fetchLatestUserData] Iniciando...");
            ...
            console.log("[fetchLatestUserData] Response status:", res.status);

            if (res.ok) {
                const latest = await res.json();
                console.log("[fetchLatestUserData] Dados recebidos:", {
                    id: latest.id,
                    email: latest.email,
            ...
            console.warn("[fetchLatestUserData] Response não ok:", res.status);
            return false;
        } catch (err) {
            console.error("[fetchLatestUserData] Erro:", err);
            return false;
        }
    }
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setUser(updatedUser);
                this.updateAvatar();
                this.showToastSuccess('toasts.profileUpdatedTitle', 'toasts.profileUpdatedMessage');
                this.closeSettings();
            } else {
    ...
    async handleDeleteAccount(e) {
        ...
        try {
            // ✅ CORRIGIDO: URL LOCAL COM API_V1
