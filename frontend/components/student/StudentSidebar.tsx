 ...
 * 
 * Client Component
 * ✅ Recebe classrooms e conversations como props
 * ✅ Usa useAuth() para dados do usuário (fonte única de verdade)
 * ✅ Gerencia filtros e collapse state localmente
 * ✅ Usa ScholarContext para selectedConversation
 * ✅ Sincroniza com mobile menu state via context
 * ✅ Inclui header com avatar do usuário
 * ✅ Inclui footer com configurações e logout
 */
...
export function StudentSidebar({
  ...
  // Gerar iniciais do usuário a partir do contexto
  ...
  return (
    <aside className={`border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-col h-full transition-all duration-300 ${
      isCollapsed 
        ? "w-16" 
        : "w-80"
    } hidden md:flex shrink-0`}>
      {isCollapsed ? (
        // 🎯 MODO COLAPSADO: Apenas ícones e avatar
        <div className="flex flex-col items-center pt-4 gap-6 flex-1">
          {/* Avatar */}
          <button
          ...
          {/* Sala + */}
          <button
            className="relative w-12 h-12 flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition"
            title="Adicionar sala"
          >
            {/* Ícone sala (PADRÃO igual sidebar aberta) */}
            <svg
            ...
            {/* + sobreposto */}
            <span className="absolute bottom-1 right-1 w-4 h-4 bg-blue-600 rounded-full flex items-center justify-center">
        ...
        <>
          {/* Header com Avatar */}
          <div className="h-16 border-b border-slate-200 dark:border-slate-800 flex items-center gap-3 px-4 flex-shrink-0">
            <button
            ...
            <button
              onClick={() => setIsCollapsed(true)}
              className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition flex-shrink-0"
          ...
          <div className="flex flex-col p-3 gap-2 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
            <div className="flex items-center justify-between px-1">
              ...
              <button
                className="flex items-center gap-2 px-2 py-1 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 transition"
              >
              ...
              {visibleClassrooms.length === 0 ? (
                ...
                <ul className="space-y-1">
                  {visibleClassrooms.map((classroom) => (
                    <li key={classroom.id} title={classroom.name}>
                      <button className="w-full text-left p-1.5 rounded hover:bg-slate-100 dark:hover:bg-slate-800 transition flex items-center gap-2">
            ...
            <ul className="flex-1 space-y-1 overflow-y-auto min-h-0">
              {visibleConversations.length === 0 ? (
                ...
                visibleConversations.map((conversation) => (
                  <li key={conversation.id} title={conversation.title || "Chat"}>
                    <button
                      onClick={() => onConversationSelect(conversation)}
                      className={`w-full text-left p-2 rounded transition flex items-center gap-2 ${
      ...
      {/* Footer com Menu - mt-auto ancora no fundo */}
      <div className="mt-auto border-t border-slate-200 dark:border-slate-800 py-2">
        ...
        {!isCollapsed && (
          <div className="flex flex-col gap-1 px-2">
            <a 
              href="/quem-somos"
              className="h-10 flex items-center gap-3 px-3 rounded-lg text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition"
            >
            ...
            <button 
              ...
              className="h-10 flex items-center gap-3 px-3 rounded-lg text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition"
            >
              <svg className="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {isCollapsed && (
          <div className="flex flex-col items-center gap-1">
            <a 
              href="/quem-somos"
              className="h-10 w-10 flex items-center justify-center rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 transition"
              title="Quem Somos"
            >
            ...
            <button 
              ...
              className="h-10 w-10 flex items-center justify-center rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500 transition"
              title="Sair"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
